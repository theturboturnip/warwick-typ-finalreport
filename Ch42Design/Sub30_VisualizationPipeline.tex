...
\begin{figure}[p]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1.2\linewidth]{Ch42Design/figures/FinalReport_Timing.pdf}}
...
The Compute sections are implemented using Vulkan Compute Shaders\todocite{vukkan compute shaders}, which are nearly equivalent to CUDA Kernels and are invoked similarly.
...
This requires a GPU reduction, which is implemented in Vulkan just like it is in CUDA, using the second kernel of \todocite{CUDA Kernel Reduction}.
...
Despite this limitation the lists are suitable to implement the technique from \todocite{https://wickedengine.net/2017/11/07/gpu-based-particle-simulation/}.
...
\todomark{Talk about timing? like synchronizing with real-time.}