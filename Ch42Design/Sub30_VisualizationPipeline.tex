...
The simulation stores data on a staggered grid (\cref{fig:staggered_grid}), but this is inconvenient for the visualization.%not amenable for some visualization tasks.
...
The second buffer is used for the reduction, containing a min/max property for each element.%, used as the input to the reduction.
