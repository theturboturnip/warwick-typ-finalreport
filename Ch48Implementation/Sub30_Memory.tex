...
\code{SimRedBlackArray} serves as a aggregate of \code{Sim2DArray}s, without defining any special behaviour.
...
\code{Sim2DArray} implements a set of functions for accessing data from the CPU, GPU, and as Vulkan handles.
...
The fixed amount is calculated by the \code{FrameSetAllocator}, and assumes only the bare minimum ($u, v, p, fluidmask$) requirements are allocated in Vulkan.
...
The visualization will use the \code{VulkanFrameSetAllocator} interface to grab references to the simulation memory, and use these references while rendering.
